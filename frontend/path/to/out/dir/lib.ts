...
export class Program {
  ...
  constructor(public api: GearApi, public programId?: `0x${string}`) {
    const types: Record<string, any> = {
      ...
      InitConfig: {"name":"String","symbol":"String","decimals":"u8","description":"String","external_links":"ExternalLinks","initial_supply":"u128","total_supply":"u128","admin":"ActorId","initial_capacity":"Option<u32>","config":"Config"},
  ...
  public createFungibleProgram(init_config: InitConfig): TransactionBuilder<{ ok: null } | { err: MemeError }> {
    return new TransactionBuilder<{ ok: null } | { err: MemeError }>(
      this.api,
      this.registry,
      'send_message',
  ...
  public subscribeToMemeCreatedEvent(callback: (data: { meme_id: number | string; meme_address: ActorId; init_config: InitConfig }) => void | Promise<void>): Promise<() => void> {
    return this.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      if (name === 'MemeCreated') {
        callback(this.registry.createType('(String, {"meme_id":"u64","meme_address":"ActorId","init_config":"InitConfig"})', message.payload)[1].toJSON() as unknown as { meme_id: number | string; meme_address: ActorId; init_config: InitConfig });
      }
    });
  }
  ...
  public subscribeToMemeRemovedEvent(callback: (data: { removed_by: ActorId; meme_id: number | string }) => void | Promise<void>): Promise<() => void> {
    return this.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      const name = this.registry.createType('String', payload.subarray(offset, limit)).toString();
      if (name === 'MemeRemoved') {
        callback(this.registry.createType('(String, {"removed_by":"ActorId","meme_id":"u64"})', message.payload)[1].toJSON() as { removed_by: ActorId; meme_id: number | string });
      }
    });
  }
  ...
  public subscribeToGasUpdatedSuccessfullyEvent(callback: (data: { updated_by: ActorId; new_gas_amount: number | string }) => void | Promise<void>): Promise<() => void> {
    return this.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      if (name === 'GasUpdatedSuccessfully') {
        callback(this.registry.createType('(String, {"updated_by":"ActorId","new_gas_amount":"u64"})', message.payload)[1].toJSON() as { updated_by: ActorId; new_gas_amount: number | string });
      }
    });
  }
  ...
  public subscribeToCodeIdUpdatedSuccessfullyEvent(callback: (data: { updated_by: ActorId; new_code_id: CodeId }) => void | Promise<void>): Promise<() => void> {
    return this.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      if (name === 'CodeIdUpdatedSuccessfully') {
        callback(this.registry.createType('(String, {"updated_by":"ActorId","new_code_id":"CodeId"})', message.payload)[1].toJSON() as { updated_by: ActorId; new_code_id: CodeId });
      }
    });
  }
  ...
  public subscribeToAdminAddedEvent(callback: (data: { updated_by: ActorId; admin_actor_id: ActorId }) => void | Promise<void>): Promise<() => void> {
    return this.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      const name = this.registry.createType('String', payload.subarray(offset, limit)).toString();
      if (name === 'AdminAdded') {
        callback(this.registry.createType('(String, {"updated_by":"ActorId","admin_actor_id":"ActorId"})', message.payload)[1].toJSON() as { updated_by: ActorId; admin_actor_id: ActorId });
      }
    });
  }
}