...
impl FungibleToken {
    ...
    fn mint(&mut self, amount: u128, to: ActorId) {
        assert!(self.admins.contains(&msg::source()), "Not admin");
        self.balances
            .entry(to)
    ...
    fn burn(&mut self, amount: u128, from: ActorId) {
        assert!(self.admins.contains(&msg::source()), "Not admin");
...
extern "C" fn handle() {
    ...
    match action {
        FTAction::Mint { amount, to } => {
            ft.mint(amount, to);
        }
        FTAction::Burn { amount, from } => {
            ft.burn(amount, from);
        }
        ...
        FTAction::AddAdmin { admin } => ft.add_admin(&admin),
     }
}
